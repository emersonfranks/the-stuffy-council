...
This pool constrains reviewer selection, and delegated-author selection
under "Delegated authoring" below. It does NOT constrain the model
authoring directly in session: `Author model` in the log records whatever
model wrote the change, in or out of the pool — the rotation rule needs
it solely to exclude that model from reviewing.

Add a model here when it becomes available rather than leaving it
unusable as a reviewer; a stale pool silently narrows the rotation.
